...
groups = set(['webserver','dbserver','idxserver','webserver','fileserver','appserver','ubuntu','oracle','sles','win25','win22','wsus'])
vm_dat = [
    {
        'name': 'vm1',
...
file = 'inventory_setdefault.yaml'
...
def create_inventory(groups, vm_obj, out_file):
    ...
    for i in range(len(vm_obj)):
        ...
        for grp in intersect:
            inventory.setdefault(grp,[]).append(vm_obj[i]['name'])
